...
public class GetSaleResultByIdQueryHandler(IMapper mapper, AppDbContext dbContext)
    : IRequestHandler<GetSaleResultByIdQuery, SaleDto>
{
    public async Task<SaleDto> Handle(GetSaleResultByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var customer = await sales.Include(x => x.SaleItems).ThenInclude(x => x.Medicine).Include(x => x.Customer)
            .FirstOrDefaultAsync(predicate: b => b.Id == request.Id, cancellationToken: cancellationToken);
